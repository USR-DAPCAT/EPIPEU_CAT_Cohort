...
## Recodificaciones y calculos 1

```{r recodificacions1 }

#general, daiagnostics
...
#vi)**temps_evolucio->TEMPS EVOLUCIO DM2
...
#vii)**temps de evolucion de la malaltia_categorica-->temps_evolucio_cat:<5,[5-11),[11-20),>=20
...
#viii)  Tiempo evoluciOn diabetes, hacer nueva categorizacion , menos de 5 anos, entre 5 y 15 anos y de 50  y mas anos de evolucion
...
#ix)  Tiempo evoluciOn diabetes, hacer nueva categorizacion , menos de 5 anos, entre 5 y 19 anos y de 20  y mas anos de evolucion
dt_plana<-dt_plana%>%mutate(temps_evolucio_cat3=case_when(
                                                      temps_evolucio<5~ 1,
                                                      temps_evolucio   >=5 &   temps_evolucio   <20 ~ 2,  
                                                      temps_evolucio>=20  ~ 3))

#x)  Tiempo evoluciOn diabetes, hacer nueva categorizacion , menos de 5 anos,  y mas 5 anos de evolucion
...
## Recodificacions i calculs 2

```{r recodificacions2}
#Tabaquisme+V.Analitiques+V.Cliniques.##

#xi) Neuropatia periferica : diagnostico registrado o el valor de monofilamento
...
#xivb)**Index de Massa Corporal->IMC.valor_cat: <15,[15-25),[25-30),>=30
...
#xv)**Index de Massa Corporal->IMC.valor_cat2: <18.5,[18.5-25),[25-30),>=30
...
#xviii)**FILTRADO GLOMERULAR :CKDEPI.valor  
...
#xix)**FILTRADO GLOMERULAR :CKDEPI.valor  
...
#xx)**FILTRADO GLOMERULAR :CKDEPI.valor  
...
#xxvi)**COLESTEROL LDL LDL.COLESTEROL.LDL, <70 , 70-99, 100-129 y >o=130
...
##xxxiii)**HBA1C.Cambiar los rangos para HbA1c (<7, [7-10), >=10)
dt_plana<-dt_plana%>%mutate(GLICADA_CAT2=case_when(GLICADA.valor   <7~ 1,
                                                   GLICADA.valor   >=7 & GLICADA.valor   <10 ~ 2,  
                                                   GLICADA.valor   >=10  ~ 3))

##xxxiv)**HBA1C.Cambiar los rangos para HbA1c (<7, [7-8),[8-) >=10)
...
##xxxv)**HBA1C.Cambiar los rangos para HbA1c <6.5 [6.5-7) [7-8), [8-9), [9-10) ,>=10
...
##xxxvi)**TRIGLICERIDOS.TG (<150, >150)
dt_plana<-dt_plana%>%mutate(TG.valor_CAT=case_when(    TG.valor   >=0 & TG.valor   <=150 ~ 1,  
                                                       TG.valor   >150  ~ 2))
...
#xl) insuficiencia renal cronica : Insuficiencia Renal Cronica: #[diagnostico +segun valores de FG, FG<60]
...
##xlii)* Nefropatia diabe?tica: diagnostico + CAC>300  mg/24h

dt_plana<-dt_plana%>%mutate(DG.NEFRPDM2=ifelse(DG.NEFRPDM==1 & CAC.valor>300 ,1,0)) 

##xlii)*Arteriopatia periferica : diagnostico registrado o el valor de ITB (<0.9)
